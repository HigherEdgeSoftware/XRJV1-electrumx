...
Not started until the Block Processor has caught up with xrjv1d.

Daemon
------

Encapsulates the RPC wire protocol with xrjv1d for the whole server.
Transparently handles temporary xrjv1d connection errors, and fails
over if necessary.
...
from xrjv1d.  Once it has caught up it additionally asks the Daemon
to refresh its view of xrjv1d's mempool transaction hashes.  Serves
blocks to the Block Processor via a queue.

Mempool
-------

Handles all the details of maintaining a representation of xrjv1d's
