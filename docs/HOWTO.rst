...
You **must** to be running a non-pruning xrjv1 daemon with::

  txindex=1

set in its configuration file.  If you have an existing installation
of xrjv1d and have not previously set this you will need to reindex
the blockchain with::
...
HDD.  Currently to around height 447,100 of the XRJV1 blockchain the
...
blocks over the LAN from a xrjv1d on machine B.  **DB_CACHE** the
default of 1,200.  LevelDB.

*Machine B*: a late 2012 iMac running Sierra 10.12.2, 2.9GHz quad-core
Intel i5 CPU with an HDD and 24GB RAM.  Running xrjv1d on the same
machine.  **DB_CACHE** set to 1,800.  LevelDB.

For chains other than xrjv1-mainnet sychronization should be much
faster.
...
Here is typical log output on startup::
  ...
  INFO:BlockProcessor:coin: XRJV1
  ...
  INFO:Controller:RPC server listening on localhost:9891
...
You may choose to run electrumx on a different port than 4191 / 4192.
If you choose a privileged port ( < 1024 ) it makes sense to make use of a iptables NAT rule.

An example, which will forward Port 110 to the internal port 4192 follows:

    iptables -t nat -A PREROUTING -p tcp --dport 110 -j DNAT --to-destination 127.0.0.1:4192
